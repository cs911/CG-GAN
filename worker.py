...
# =========================================================
        elif cfgs.DATA.name in ["ImageNet", "Baby_ImageNet", "Papa_ImageNet", "Grandpa_ImageNet"]:
            self.num_eval = {"train": 50000, "valid": len(eval_dataloader.dataset)}
            # self.num_eval = {"train": 50000, "valid": 10000}
            
        # elif self.DATA.name == "Grandpa_ImageNet":
        #     self.num_eval = {"train": 50000, "valid": len(eval_dataloader.dataset)}
        elif self.DATA.name == "CUB200":
            self.num_eval = {"train": len(self.train_dataloader.dataset), "valid": len(eval_dataloader.dataset)}    
            # self.num_eval = {"train": 50000, "valid": 10000}
...
# =========================================================
    ...
    def train_discriminator(self, current_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    fake_images, fake_labels, fake_images_eps, trsp_cost, ws, _, _ = sample.generate_images(
                        ...
                        num_classes=self.DATA.num_classes,
                        y_sampler=self.LOSS.y_sampler,

                        # ==================================
                        # fake_pair = real_labels,
                        # adv_noise = self.LOSS.adv_noise,
                    ...
                    # if self.LOSS.adv_noise:
                    #     fake_images1_ = self.AUG.series_augment(fake_images1)
                    #     fake_dict1 = self.Dis(fake_images1_, fake_labels)
                    ...
                    real_mean_norm =torch.mean(torch.norm(real_dict["h"],p=2,dim=1))    
                    fake_mean_norm =torch.mean(torch.norm(fake_dict["h"],p=2,dim=1))  

                    # calculate class conditioning loss defined by "MODEL.d_cond_mtd"
                    if self.MODEL.d_cond_mtd in self.MISC.classifier_based_GAN:

                        
                        if self.MODEL.aux_cls_type == "NKL":
     
                            label_g = torch.unsqueeze(fake_labels, 1)
                            fake_adv = fake_dict["cls_output"].gather(1, label_g)
                            fake_adv_ood = 1/self.LOSS.tau * torch.logsumexp(self.LOSS.tau * fake_dict["cls_output"], dim=1)
                            # fake_adv_ood = torch.logsumexp(fake_dict["cls_output"], dim=1)
                            
                            label_d = torch.unsqueeze(real_labels, 1)
                            real_adv = real_dict["cls_output"].gather(1, label_d)
                            real_adv_ood = 1/self.LOSS.tau * torch.logsumexp(self.LOSS.tau * real_dict["cls_output"], dim=1)
                            # real_adv_ood = torch.logsumexp(real_dict["cls_output"], dim=1)
                            
                            
                            soft_output = nn.Softmax(dim=1)
                            softmax_fake = soft_output(fake_dict["cls_output"])
                            current_batch = softmax_fake.shape[0]
                            
                            real_cond_loss = -torch.mean(real_adv - real_adv_ood) + torch.mean(F.relu(-math.log(self.LOSS.pro) + (fake_adv - fake_adv_ood)))
                            
                            fake_cond_loss = losses.NegCrossEntropy(self.LOSS.pro, label_g, current_batch, self.DATA.num_classes)
                            real_cond_loss += self.LOSS.reg_lambda * fake_cond_loss(fake_dict["cls_output"])
                            

                            dis_acml_loss += self.LOSS.cond_lambda * (real_cond_loss)
 
                        elif self.MODEL.aux_cls_type == "CCG":
                            ...
                            dis_acml_loss += self.LOSS.cond_lambda * (real_cond_loss) + self.LOSS.reg_lambda * fake_cond_loss
                        elif self.MODEL.aux_cls_type == "LAB":
        
                            label_g = torch.unsqueeze(fake_labels, 1)
                            fake_adv = fake_dict["cls_output"].gather(1, label_g)
                            fake_adv_ood = torch.logsumexp(fake_dict["cls_output"], dim=1)

                            real_cond_loss = self.cond_loss(**real_dict) + torch.mean(F.relu(-math.log(self.LOSS.pro) + (fake_adv - fake_adv_ood)))
                            
                            fake_cond_loss = losses.LabelCrossEntropy(self.LOSS.pro, self.DATA.num_classes)
                            real_cond_loss += self.LOSS.reg_lambda * fake_cond_loss(fake_dict["cls_output"], fake_labels)
        ...
        # wight_info = real_dict["cos_dict"]
        wight_info = None
        


        return real_cond_loss, dis_acml_loss, wight_info, fake_entropy, real_entropy, real_mean_norm, fake_mean_norm
                ...
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    if self.MODEL.d_cond_mtd in self.MISC.classifier_based_GAN:
                        ...
                        if self.MODEL.aux_cls_type == "NKL":
                            ...
                            fake_cond_loss = -torch.mean(fake_adv - fake_adv_ood)
                            
                            
                            
                            fake_reg_loss = losses.NegCrossEntropy(self.LOSS.pro, label_g, current_batch, self.DATA.num_classes)
                            fake_cond_loss -= self.LOSS.reg_lambda * fake_reg_loss(fake_dict["cls_output"])
                            ...
                            fake_reg_loss = losses.LabelCrossEntropy(self.LOSS.pro, self.DATA.num_classes)
                            fake_cond_loss -= self.LOSS.reg_lambda * fake_reg_loss(fake_dict["cls_output"], fake_labels)
    ...
    # -----------------------------------------------------------------------------
    def log_train_statistics(self, current_step, real_cond_loss, gen_acml_loss, dis_acml_loss, wight_info, fake_entropy, real_entropy, real_mean_norm, fake_mean_norm):
        self.wandb_step = current_step + 1

        real_mean_norm = real_mean_norm
        fake_mean_norm = fake_mean_norm
        # if fake_entropy:

        #     # adv_cos = wight_info['adv_cos']
        #     # adv_norm0 = wight_info['adv_norm0']
        #     # adv_norm1 = wight_info['adv_norm1']
        ...
        if self.MODEL.d_cond_mtd in self.MISC.classifier_based_GAN:
            ...
            # if self.MODEL.aux_cls_type == "NLL_WOOG":
            #     cls_loss = "N/A"
            # else:
            ...
            for i, (images, labels) in enumerate(self.train_dataloader):
                if GAN_train:
                    images, labels, _, _, _, _, _ = sample.generate_images(z_prior=self.MODEL.z_prior,
                                                                     ...
                                                                     # ==================================
                                                                    # fake_pair = labels.to(self.local_rank),
                                                                    # adv_noise = self.LOSS.adv_noise,
