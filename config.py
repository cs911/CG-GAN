...
class Configurations(object):
    ...
    def load_base_cfgs(self):
        ...
        # type of auxiliary classifier \in ["W/O", "TAC", "ADC", 'PAIR]
        ...
        # ==============================================
        # self.LOSS.mydef = False

        self.LOSS.transition = 1.0
        self.LOSS.adv_lambda = 1.0
        self.LOSS.ec_lambda = 1.0
        self.LOSS.ova_lambda = 1.0
        self.LOSS.pro = "N/A"
        self.LOSS.tau= 1.0
        
        self.LOSS.adv_pro = "N/A"
        self.LOSS.reg_lambda = 1.0
        
        self.LOSS.energy = False
        self.LOSS.m_in = -7
        self.LOSS.m_out = -25

        self.LOSS.logitnorm = False
        self.LOSS.logit_temp = 1.0

        self.LOSS.rs_type = "N/A"
        self.LOSS.ood_lambda = 1.0
        
        self.LOSS.cls_logit = False
        self.LOSS.cls_logit_temp = 1.0
        
        self.LOSS.G_LabelSmooth = False
        self.LOSS.D_LabelSmooth = False
        self.LOSS.G_Smooth_type = "N/A"
        self.LOSS.D_Smooth_type = "N/A"

        self.LOSS.smoothing = 0.1

        self.LOSS.center = False
        self.LOSS.center_lambda = 1.0

        self.MODEL.d_adv_mtd = "N/A"
        self.MODEL.normalize_adv_embed = False


        #  adversarialv loss
        self.LOSS.y_sampler = 'totally_random'  # class_pair // totally_random
        self.LOSS.adv_noise = False
    ...
    #   ===================================================================
        elif self.LOSS.adv_loss == "acg":
            self.LOSS.g_loss = losses.g_acg
            self.LOSS.d_loss = losses.d_acg
        
        elif self.LOSS.adv_loss == "soft":
            self.LOSS.g_loss = losses.g_soft
            self.LOSS.d_loss = losses.d_soft

        elif self.LOSS.adv_loss == "rs":
            self.LOSS.g_loss = losses.rs_gloss
            self.LOSS.d_loss = losses.rs_dloss
    ...
    #   ===================================================================      
        ...
        if self.MODEL.class_adv_model: 
            if self.MODEL.d_cond_mtd == "MH" and self.LOSS.class_adv_loss == "MH":
                self.LOSS.gc_loss = losses.crammer_singer_loss
                self.LOSS.dc_loss = losses.crammer_singer_loss
        #   ===================================================================
            elif self.LOSS.class_adv_loss == "soft":

                self.LOSS.gc_loss = losses.g_soft
                self.LOSS.dc_loss = losses.d_soft
            
            elif self.LOSS.class_adv_loss == "rs":
                self.LOSS.gc_loss = losses.rs_gloss
                self.LOSS.dc_loss = losses.rs_dloss
        ...
        #   ===================================================================      
                ...
                dc_losses = {
                    ...
                    "least_square": losses.d_ls,
                    "hinge": losses.d_hinge,
                    # "shinge": losses.d_shinge,

                    "wasserstein": losses.d_wasserstein,

                }

                self.LOSS.gc_loss = gc_losses[self.LOSS.class_adv_loss]
                self.LOSS.dc_loss = dc_losses[self.LOSS.class_adv_loss]
    ...
    def check_compatability(self):
        ...
        if isinstance(self.MODEL.g_shared_dim, int):
            assert self.MODEL.backbone in ["big_sym","big_resnet", "big_resnet_deep_legacy", "big_resnet_deep_studiogan"], \
        ...
        if isinstance(self.MODEL.g_conv_dim, int) or isinstance(self.MODEL.d_conv_dim, int):
            assert self.MODEL.backbone in ["big_sym","simple_resnet", "resnet_simple", "resnet_sym", "simple_model", "resnet", "big_resnet", "big_resnet_deep_legacy", "big_resnet_deep_studiogan"], \
